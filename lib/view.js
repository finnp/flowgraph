...
FlowGraphView.prototype.activateEdge =function (source, target) {
  var selector = '.edge-' + source + '-' + target
  var edge = document.querySelector(selector)
  classList(edge).add('edgeactive')
}

FlowGraphView.prototype.deactivateEdge =function (source, target) {
  var selector = '.edge-' + source + '-' + target
  var edge = document.querySelector(selector)
  classList(edge).remove('edgeactive')
}

FlowGraphView.prototype.blinkEdge = function(source, target, ms) {
  ms = ms || 300
  this.activateEdge(source, target)
  setTimeout(this.deactivateEdge.bind(this, source,target), ms)
}
...
FlowGraphView.prototype.registerHandlers = function() {
  ...
  delegate.on(this.svg, '.node circle', 'mouseover', function (e) {
    ...
    var fromId = data.connector.from.id
